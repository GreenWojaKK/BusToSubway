...
_LEGACY_PUBLISHED_STATUS = "pro" + "moted"
...
def is_published_status(status: str | None) -> bool:
    """게시 상태 판정. 이전 manifest 값도 읽기 호환한다."""
    return status in {PUBLISHED_STATUS, _LEGACY_PUBLISHED_STATUS}
...
def resolve_inputs(stage: str, scope: str, pins: dict[str, str] | None = None) -> ResolvedInputs:
    ...
    """
    from run import REGISTRY  # 지연 import (순환 회피)
    pins = pins or {}
    st = REGISTRY[stage]
    ri = ResolvedInputs()
    for up in st.inputs:
    ...
    cfg_files = (["src/config/params.yaml"]
                 + _FROZEN_LAYER_FILES
...
def find_published(stage: str, scope: str, key: str) -> str | None:
    ...
    for d in sorted(sdir.iterdir()):
        if not d.is_dir() or not (d / "manifest.json").exists():
            continue
        m = read_manifest(d)
        if m.get("content_key") == key and is_published_status(m.get("status")):
            return m["version"]
    return None
...
def write_manifest(vdir: Path, stage: str, scope: str, inputs: ResolvedInputs,
    ...
    'override 1행 이상 실린 상태로 빌드된' 여부). publish가 현재 디스크 파일 대신
...
def publish(stage: str, scope: str, vdir: Path) -> None:
    """_latest.json 원자 갱신 (publish 경로만)."""
