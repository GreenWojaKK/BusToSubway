...
raw 대조가 필요한 체크(A-001~004, A-011)는 dataio 로더를 경유해 raw를 읽는다(io 관문 규약).
...
from dataio import raw_bus_route_after, raw_schedule_after, timeparse
...
def c003_arrival_format(arrival: pd.Series) -> core.CheckResult:
    return core.regex_all("C-S00-A-003", "CONTRACT", arrival, timeparse.KR_TS_RE.pattern,
                          ...
                          note="오전12=0시, 오후12=12시 — 파서는 dataio.timeparse.parse_kr_ampm")
