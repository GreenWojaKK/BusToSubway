"""스테이지 러너 — python -m run <stage> --scope <s> [--pin ...] (design.md §4.3, §6).
...
REGISTRY: dict[str, Stage] = {
    "s00_ingest": Stage(
        builders={"before": "s00_ingest.before:build",
                  "after": "s00_ingest.after:build"},
        inputs=[],                                   # 상류 artifact 없음 (raw는 input_files 핀)
        checks={"before": "checks.contracts.s00_before",
                "after": "checks.contracts.s00_after"},
        config_files=["src/config/overrides/name_aliases.csv"],
    ...
    "s01_canonical": Stage(
        builders={"before": "s01_canonical.before:build",
                  "after": "s01_canonical.after:build"},
        inputs=["s00_ingest"],                       # ★ 입력 화이트리스트
        checks={"before": "checks.contracts.s01_before",
                "after": "checks.contracts.s01_after"},
        config_files=["src/config/route_class_rules.{scope}.yaml"],
        input_files={                                # 수동 분류표 직접 읽기 입력 (design.md §4.2 예시)
            "before": ["reference/variant_tagging/variant_tags.csv",
                       "reference/variant_tagging/evidence"]}),
    "s02_place": Stage(
        builders={"before": "s02_place.merge:build_before",
                  "after": "s02_place.merge:build_after"},
        inputs=["s00_ingest"],                       # stops.parquet만 소비 — s01과 독립(병렬 가능)
        checks={"before": "checks.contracts.s02_before",
                "after": "checks.contracts.s02_after"},
        config_files=["src/config/overrides/place_overrides.{scope}.csv",
                      "src/config/overrides/name_aliases.csv"],
        # raw 직접 읽기 없음 → input_files 미선언 (stops는 s00 artifact — spec §3)
        review_overrides=["src/config/overrides/place_overrides.{scope}.csv"]),
    "s03_hub": Stage(
        builders={"before": "s03_hub.metrics:build_before"},
        inputs=["s01_canonical", "s02_place"],   # backbone·catalog·(주석용)patterns + 맵/places
        checks={"before": "checks.contracts.s03_before"},
        config_files=["src/config/overrides/hub_overrides.csv"],
        review_overrides=["src/config/overrides/hub_overrides.csv"],
...
def _ack_entry(check_id: str, reviewed_by: str) -> dict:
    ...
    익명 ack은 존재하지 않는다 — ack 적용 경로(run/publish)가 --reviewed-by 부재 시
...
def _needs_review(st: Stage, scope: str) -> bool:
    ...
    게시 경로(publish)는 이 함수를 직접 쓰면 안 된다 — 빌드~publish 사이 파일 원복으로
    게이트가 우회되는 TOCTOU가 생긴다. publish는 _version_needs_review(버전 자신의 기록)로
...
def run(stage: str, scope: str, pins: dict | None = None, acks: list | None = None,
    ...
    if stage not in REGISTRY:
        print(f"[run] 미등록 스테이지: {stage} (registry: {sorted(REGISTRY)})")
        return EXIT_UPSTREAM
    st = REGISTRY[stage]
    if scope not in st.scopes:
        print(f"[run] {stage}에 없는 scope: {scope}")
        return EXIT_UPSTREAM
    ...
    try:
        ...
        print(f"[run] 상류 입력 확인 실패 (exit {EXIT_UPSTREAM}): {e}")
    ...
    if existing:
        ...
        if current != existing:
            ...
            print(f"[run] 재사용 대상({existing})이 _latest({current})와 달라 "
                  f"_latest를 동기화했다")
        print(f"[run] 기존 게시본 재사용: {stage}/{scope} — 동일 content_key {existing}")
    ...
    # review 게이트 술어는 빌드 시점에 1회 판독해 manifest에 고정 기록한다 — publish가
    ...
    try:
        with paths.build_context(vdir):
            ...
            if approved_sanity_ids and not reviewed_by:
                ...
                print(f"[run] 익명 ack 미적용: --ack {approved_sanity_ids}는 --reviewed-by <이름>과 "
                      f"함께만 적용된다 — 보류")
            ...
            if blocking_failures:
                ...
                status = manifest.PUBLISHED_STATUS
        ...
        print(f"[run] 빌드 중 ContractViolation → rejected 처리 (exit {EXIT_CONTRACT}) — "
              f"보존: {rej.name}\n  {e}")
    ...
    if blocking_failures:
        ...
        print(f"[run] CONTRACT FAIL {ids} → rejected 처리 (exit {EXIT_CONTRACT}) — 보존: {rej.name}")
    ...
    if unapproved_sanity_failures:
        ...
        print(f"[run] PHYSICAL 미승인 {ids} → 보류 (exit {EXIT_PHYSICAL}). "
              f"재개: python -m run publish {stage} {scope} {vdir.name} --ack <id>")
        _record_run(stage, scope, EXIT_PHYSICAL, vdir.name, key, f"sanity_check_unapproved:{ids}")
        return EXIT_PHYSICAL

    if review_block:
        print(f"[run] 판단 개입 산출물(override 적용) — --reviewed-by 필요, 보류 (exit {EXIT_PHYSICAL})")
    ...
    if unexplained_baseline_diffs:
        ...
        print(f"[run] 게시 + 신규 DIFF UNEXPLAINED {ids} (exit {EXIT_UNEXPLAINED}) — 조사 메모 생성됨")
        _record_run(stage, scope, EXIT_UNEXPLAINED, vdir.name, key, f"unexplained:{ids}")
        return EXIT_UNEXPLAINED

    print(f"[run] {stage}/{scope} {vdir.name} 게시 (exit {EXIT_OK})")
    _record_run(stage, scope, EXIT_OK, vdir.name, key, "published")
    return EXIT_OK
...
def publish_pending(stage: str, scope: str, version: str, acks: list,
    ...
    - 게시 직전 출력 해시 재검증(verify_outputs) — 빌드~publish 사이 출력 변경은 exit 5.
    ...
    if acks and not reviewed_by:
        print(f"[run] 익명 ack 미적용: --ack {acks}는 --reviewed-by <이름>과 함께만 "
              f"적용된다 (exit {EXIT_PHYSICAL})")
        _record_run(stage, scope, EXIT_PHYSICAL, version, key,
                    f"publish_anonymous_ack_refused:{acks}")
        return EXIT_PHYSICAL
    if manifest.is_published_status(m.get("status")):
        print(f"[run] 이미 게시됨: {stage}/{scope}/{version}")
        _record_run(stage, scope, EXIT_OK, version, key, "publish_already_published")
        return EXIT_OK
    if m.get("status") == "rejected":
        print(f"[run] rejected 버전은 게시 불가 — 재실행하라")
        _record_run(stage, scope, EXIT_CONTRACT, version, key, "publish_rejected_version")
        return EXIT_CONTRACT
    bad = manifest.verify_outputs(vdir, m)
    if bad:
        print(f"[run] 산출물 해시 불일치(빌드 후 출력 변경) {bad} — 게시하지 않음 "
              f"(exit {EXIT_UPSTREAM}), 재실행하라")
        _record_run(stage, scope, EXIT_UPSTREAM, version, key,
                    f"publish_outputs_changed:{bad}")
        return EXIT_UPSTREAM
    ...
    if unapproved_sanity_ids:
        print(f"[run] PHYSICAL 미승인 잔존 {unapproved_sanity_ids} (exit {EXIT_PHYSICAL})")
        _record_run(stage, scope, EXIT_PHYSICAL, version, key,
                    f"publish_sanity_check_unapproved:{unapproved_sanity_ids}")
        return EXIT_PHYSICAL
    if _version_needs_review(m, REGISTRY[stage], scope) and not reviewed_by:
        print(f"[run] 판단 개입 상태로 빌드된 버전 — --reviewed-by 필요 (exit {EXIT_PHYSICAL})")
        _record_run(stage, scope, EXIT_PHYSICAL, version, key, "publish_needs_reviewed_by")
        return EXIT_PHYSICAL
    ...
    if ignored:
        print(f"[run] --ack 중 이 버전의 PHYSICAL FAIL이 아닌 항목은 기록하지 않는다: {ignored}")
    ...
    m["status"] = manifest.PUBLISHED_STATUS
    ...
    if unexplained_baseline_diffs:
        print(f"[run] {stage}/{scope}/{version} 게시 + UNEXPLAINED DIFF {unexplained_baseline_diffs} "
              f"(exit {EXIT_UNEXPLAINED}) — 조사 메모는 빌드 시점에 생성됨")
        _record_run(stage, scope, EXIT_UNEXPLAINED, version, key,
                    f"published_via_publish_unexplained:acks={approved_sanity_ids}:{unexplained_baseline_diffs}")
        return EXIT_UNEXPLAINED
    print(f"[run] {stage}/{scope}/{version} 게시 (--ack {approved_sanity_ids}, reviewed_by={reviewed_by})")
    _record_run(stage, scope, EXIT_OK, version, key,
                f"published_via_publish:acks={approved_sanity_ids}")
    return EXIT_OK
...
def recheck(scope: str) -> int:
    """게시본 전체 재검증 (출력 변경·회귀 확인) — python -m run checks --scope s."""
    worst = EXIT_OK
    for s in _topo_order():
        try:
...
def main(argv=None) -> int:
    ...
    ap.add_argument("stage",
                    help="스테이지명 | all | checks | publish (registry: %s)" % ", ".join(REGISTRY))
    ap.add_argument("rest", nargs="*",
                    help="publish 서브커맨드 전용: <stage> <scope> <version>")
    ...
    legacy_publish = "pro" + "mote"
    if args.stage in {"publish", legacy_publish}:
        if len(args.rest) != 3:
            ap.error("publish는 <stage> <scope> <version> 3인자가 필요하다")
