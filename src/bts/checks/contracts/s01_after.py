...
동결 상수가 기본값이고, 합성 테스트는 자기 데이터에 맞는 기대치를 주입한다.
...
사전 실측 주(2026-07-04, s00 v001 위 재현 — 구현 판별 근거):
  ...
  - route_class 관측 분포는 yangsan 5(13·2100·2300·3000·3100 — 감사 §6.2 열거와 일치)다.
    ...
    정본은 yaml이고, 경성 항등(합계 184)은 감사 상수로 이중 고정한다.
"""
...
from bts.io import raw_schedule_after
from bts.stages.s01_canonical import trip_split
from bts.stages.s01_canonical.after import get_rules

# ── 감사 실측 동결 상수 (원시 로그의 검증 규칙 — 튜닝 대상 아님. 출처를 각 체크 source에 기입) ──
...
# 클래스 분포 기대치는 코드 상수가 아니라 rules yaml의 expect_names가 정본이다
...
# ── C-S01-A-007: 정차열 정본 = route_master 승계 (stop_sequence 미사용) ───────
def c007_route_master_inherited(out_path: Path, upstream_path: Path,
    ...
    return core.check_true(
        ...
        note="dense_rank_*_ref는 참고 기록(감사 94% — 게이트 아님). 정차열 정본은 "
...
def c008_route_class(route_registry: pd.DataFrame, rules: dict,
    ...
    # 분포 기대치의 정본 = rules yaml의 expect_names (첫 실행 확정치 게시 — design.md §7.2).
    ...
    return core.check_true(
        ...
        note="경성 항등 = 클래스 합 == 184. 분포 기대치의 정본은 rules yaml expect_names "
...
# ── C-S01-A-009: main 제안 — 이중 규칙 합의·강제 판정 0 (ADR-003) ────────────
def c009_main_proposal(proposal: pd.DataFrame,
    ...
    return core.check_true(
        ...
        note="합의(variant_code==0 ∧ 최다 trip)만 main_candidate 확정 — 불일치는 "
             "needs_review=True 보존(강제 판정 금지). agreement_rate·basis_counts는 "
...
def run(ctx) -> list[core.CheckResult]:
    ...
    rules = get_rules(ctx.scope)                   # era 명시 조회 — 무스코프 금지
